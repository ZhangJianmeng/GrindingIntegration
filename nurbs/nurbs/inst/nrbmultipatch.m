...
for i=1:numel(boundary)
    crvss=nrbextract(nurbs(boundary(i).patches));
    cc=crvss(boundary(i).faces);
    cc.coefs(1,:)=cc.coefs(1,:)-cc.coefs(1,1);
    cc.coefs(2,:)=cc.coefs(2,:)-cc.coefs(2,1);
    cc.coefs(3,:)=cc.coefs(3,:)-cc.coefs(3,1);
    cc.coefs(4,:)=cc.coefs(4,:)-cc.coefs(4,1);
    sumcc=sum(cc.coefs(:));
    if abs(sumcc)<0.0001
       j=j+1;
        a(j)=i; 
